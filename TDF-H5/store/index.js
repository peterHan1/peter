...
export const actions = {
  async getNoticeList({ commit }, params) {
    ...
    data.content.dataRows.map(o => {
      commit('setNoticeData', o)
    })
    commit('setNoticePages', data.content.pages)
  },
  // 动态列表
  async getDynamicList({ commit }, params) {
    let { data } = await homeNoticeDynamic(this.$axios, params)
    data.content.dataRows.map(o => {
      commit('setDynamicData', o)
    })
    // commit('setDynamicPages', data.content.pages)
  },
  // 平台7日成交量数据
