<template>
  <cube-scroll
    ref="contentScroll0"
    :data="this.$store.state.project.freeData"
    :options="options"
    ...
    <ul class="pro-ul">
      <li
        v-for="(item, index) in this.$store.state.project.freeData"
        v-if="item.rate!=100"
        :key="index">
        <router-link
          to="/project/free/details/scatterDetails/indexs">
          <div>
    ...
    <ul class="pro-ul pro-invalid">
      <li
        ...
        v-if="item.rate==100"
        :key="index">
        <nuxt-link to="/project/free/details">
          <div class="pro-name">{{ item.name }}<tags :type="item.typeId" /></div>
          <dl class="pro-con">
            <dd>{{ item.apr }}<i>%</i><i v-if="item.platformApr!=0">+{{ item.platformApr }}%</i></dd>
            <dd>{{ item.period }}个月</dd>
