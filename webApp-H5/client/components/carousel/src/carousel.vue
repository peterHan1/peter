<template>
  <div class="carousel" ref="carouselOut"
    @touchstart="touchStart($event)">
    <div class="carouselInner" :style="{ transform: 'translate3d('+ distance + 'px, 0, 0)'}" :class="{active: active}">
      <img ref="img1" :style="{ transform: 'translate3d('+ imgDis1 + 'px, 0, 0) scale('+scale1+')', opacity: (scale1-1+0.6)}" :class="{active: active}" :src="imgs.src1" alt="">
      <img ref="img2" :style="{ transform: 'translate3d('+ imgDis2 + 'px, 0, 0) scale('+scale2+')', margin: '0 '+offset+'px', opacity: (scale2-1+0.6)}" :class="{active: active}" class="img2" :src="imgs.src2" alt="">
      <img ref="img3" :style="{ transform: 'translate3d('+ imgDis3 + 'px, 0, 0) scale('+scale3+')', opacity: (scale3-1+0.6)}" :class="{active: active}" :src="imgs.src3" alt="">
    </div>
    <slot></slot>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    touchEnd () {
      ...
      window.ontouchend = function (e) {
        ...
        if (that.distance <= -maxDis) {
          ...
          that.indexChange(3)
        } else if (that.distance < that.distanceStart && that.distance < 0 && that.distance > -maxDis) {
          that.distance = -maxDis
          that.indexChange(3)
        } else if (that.distance > -maxDis && that.distance <= 0 && that.distance > that.distanceStart) {
          that.distance = 0
          that.indexChange(2)
        } else if (that.distance < maxDis && that.distance >= 0 && that.distance < that.distanceStart) {
          that.distance = 0
    ...
    change () {
      ...
      let offsets = (that.outWidth - that.$refs.img1.width * 3 - that.offset * 2) / 2 + that.$refs.img1.width
      ...
      // 图形整体向左移动
        ...
        // that.imgDis2 = -dis2 / maxDis * (maxDis - offsets - that.$refs.img1.width * 1.4 / 2)
      } else {
        that.scale1 = dis1 / maxDis * 0.4 + 1
        that.scale3 = dis1 / maxDis * 0.4 + 1
  ...
  mounted () {
    ...
    this.maxDis = this.$refs.img1.width + this.offset
    ...
    window.onresize = function () {
      ...
      that.maxDis = that.$refs.img1.width + that.offset
    }
  }
}
</script>
