<template>
  <div class="citySelect" v-show="show">
    ...
    <transition name="move"  @before-enter="beforeEnter" @enter="enter" @leave="leave" @after-leave="afterLeave">
      <div class="citySelect-con" v-if="dateShow">
        ...
        <div class="citySelect-bottom">
          <div class="citySelect-inner citySelect-province" @touchstart="touchStartProvince($event)">
            ...
            <ul :style="{transform: 'perspective(1000px) rotateY(0deg) rotateX('+angle1+'deg)'}" :class="{active: active1}">
              <li v-for="(item, index) in year" :key="index" style="transform-origin: center center -89.5px;" :style="{transform: 'translateZ(89.5px) rotateX('+(-20*index)+'deg)'}" :class="[index === highLight1?'highLight':'', (index >= (showIndex1-4) && index<=(showIndex1+4))?'visible':'']" @click.stop="clickCity(index, 'province')">{{item.year}}</li>
            </ul>
          </div>
          <div class="citySelect-inner citySelect-city" @touchstart="touchStartCity($event)">
            ...
            <ul :style="{transform: 'perspective(1000px) rotateY(0deg) rotateX('+angle2+'deg)'}" :class="{active: active2}">
              <li v-for="(item, index) in months" :key="index" style="transform-origin: center center -89.5px;" :style="{transform: 'translateZ(89.5px) rotateX('+(-20*index)+'deg)'}" :class="[index === highLight2?'highLight':'', (index >= (showIndex2-4) && index<=(showIndex2+4))?'visible':'']" @click.stop="clickCity(index, 'city')"><nobr>{{item.month}}</nobr></li>
            </ul>
          </div>
          <div class="citySelect-inner citySelect-area" @touchstart="touchStartArea($event)">
            ...
            <ul :style="{transform: 'perspective(1000px) rotateY(0deg) rotateX('+angle3+'deg)'}" :class="{active: active3}">
              <li v-for="(item, index) in days"  :key="index" style="transform-origin: center center -89.5px;" :style="{transform: 'translateZ(89.5px) rotateX('+(-20*index)+'deg)'}" :class="[index === highLight3?'highLight':'', (index >= (showIndex3-4) && index<=(showIndex3+4))?'visible':'']" @click.stop="clickCity(index, 'area')"><nobr>{{item}}</nobr></li>
            </ul>
          </div>
        </div>
      </div>
...
export default {
  ...
  data () {
    return {
      ...
      length1: 0, // 省份li标签的总长度（年份个数）
      highLight1: 0, // 年份高亮时的下标，用来控制字体颜色变黑高亮
      months: this.$myDate[0].children,
      angle2: 0,
      angleStart2: 0,
      active2: false,
      showIndex2: 0,
      length2: 0,
      highLight2: 0, // 市高亮
      days: this.$myDate[0].children[0].children,
      angle3: 0,
      angleStart3: 0,
      active3: false,
      showIndex3: 0,
  ...
  mounted () {
    this.length1 = this.year.length
    this.length2 = this.months.length
    this.length3 = this.days.length
  },
  methods: {
    getValue () {
      this.$emit('dateValue', this.value)
    },
    cancel () {
      this.$emit('on-close')
    },
    select () {
      this.value = this.$myDate[this.showIndex1].year + '-' + this.$myDate[this.showIndex1].children[this.showIndex2].month + '-' + this.$myDate[this.showIndex1].children[this.showIndex2].children[this.showIndex3]
      this.getValue()
      this.$emit('on-close')
    },
    beforeEnter () {
    ...
    changeCity () {
      if (this.showIndex2 >= this.$myDate[this.showIndex1].children.length - 1) {
        this.showIndex2 = this.$myDate[this.showIndex1].children.length - 1
      }
      if (this.showIndex3 >= this.$myDate[this.showIndex1].children[this.showIndex2].children.length - 1 && this.$myDate[this.showIndex1].children[this.showIndex2].children !== undefined) {
        this.showIndex3 = this.$myDate[this.showIndex1].children[this.showIndex2].children.length - 1
      } else if (this.$myDate[this.showIndex1].children[this.showIndex2].children === undefined) {
        this.days = []
      }
      this.months = this.$myDate[this.showIndex1].children
      this.days = this.$myDate[this.showIndex1].children[this.showIndex2].children
      this.length2 = this.months.length
      this.length3 = this.days.length
