!function(a) {
	...
	a.laydate = function(b) {
		...
		var f = d[e]("link");
		// f.type = "text/css", f.rel = "stylesheet", f.href = c.getPath + a, b && (f.id = b), d[g]("head")[0].appendChild(f);
	}, c.trim = function(a) {
		...
		$(".layer_date").removeClass('laydate_on');
		c.addClass(c.elem, 'laydate_on');
		...
		var i, g = c.query,
			j = {};
		f = f || a, c.elem = a, c.options = f, c.options.format || (c.options.format = b.format), c.options.start = c.options.start || "", c.mm = j.mm = [c.options.min || b.min, c.options.max || b.max], c.mins = j.mm[0].match(/\d+/g), c.maxs = j.mm[1].match(/\d+/g), h.elemv = /textarea|input/.test(c.elem.tagName.toLocaleLowerCase()) ? "value" : "innerHTML", c.box ? c.shde(c.box) : (i = d[e]("div"), i.id = h[0], i.className = h[0], i.style.cssText = "position: absolute;", i.setAttribute("name", "laydate-v" + laydate.v), i.innerHTML = j.html = '<div class="laydate_top"><div class="laydate_ym laydate_y" style="display:none;" id="laydate_YY"><a class="laydate_choose laydate_chprev laydate_tab"><cite></cite></a></div><div class="laydate_ym laydate_m" id="laydate_MM"><a class="laydate_choose laydate_chprev laydate_tab iconfont">&#xe6bb;</a><div class="laydate_txt"><input id="laydate_y" readonly><input id="laydate_m" readonly></div><a class="laydate_choose laydate_chnext laydate_tab iconfont">&#xe6ba;</a><div class="laydate_yms" id="laydate_ms" style="display:none;"></div>' + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom" style="display:none">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">时间</li>' + "<li><input readonly>:</li>" + "<li><input readonly>:</li>" + "<li><input readonly></li>" + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<a id="laydate_today">今天</a>' + '<a id="laydate_ok">确认</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>" : "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null), c.follow(c.box), f.zIndex ? c.box.style.zIndex = f.zIndex : c.removeCssAttr(c.box, "z-index"), c.stopMosup("click", c.box), c.initDate(), c.iswrite(), c.check();
	}, c.reshow = function() {
		return c.each(c.query("#" + h[0] + " .laydate_show"), function(a, b) {
			c.removeClass(b, "laydate_show");
		}), this;
	}, c.close = function() {
		c.removeClass(c.elem, 'laydate_on');
		...
		if (c.hasClass(c.elem, 'laydate_on')) {
			c.removeClass(c.elem, 'laydate_on');
		}
		...
		c.use(h[4] + b.defSkin, h[3]), c.skinLink = c.query("#" + h[3]);
