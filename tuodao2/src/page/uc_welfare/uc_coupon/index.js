...
var coupon = {
	...
	getDyqCoupon : function(type,status,lock){
		...
		_apiOperate.getCoupon(coupon.headerData,data,function(res){
	...
	tabCut : function(){
		...
		$('.coupon_menu a').on("click",function(){
			...
			if (index == 0) {
				$('.dyq_state_menu a').eq(0).addClass('menu_on').siblings().removeClass('menu_on');
				coupon.getDyqCoupon(1,1,1);
			} else {
				$('.jxq_state_menu a').eq(0).addClass('menu_on').siblings().removeClass('menu_on');
