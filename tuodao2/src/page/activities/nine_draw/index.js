...
var lottery = {
	init : function(){
		...
		this.draw_Btn2();
		this.clickEvnet();
		this.stopEvent('#go_draw_10');
		this.stopEvent('#go_draw_100');
	},
	...
	layerShow :function(obj){
		lottery.timers = setTimeout(function(){
			lottery.layerPublic(obj);
		    clearTimeout(lottery.timers);
		},lottery.alertTime);
	},
	drawPointsYes : function(score){
		if (score < 10) {
			$('#draw_points .color9e span').html('不要泄气，小奖怡情');
		} else {
			$('#draw_points .color9e span').html('不错不错，再接再厉！');
		}
		$('#draw_points .color333 span').html(score);
		lottery.layerShow('draw_points');
	},
    stop1 : function(){
        lottery.times += 1;
        ...
        if (lottery.times > lottery.cycle+10 && lottery.prize==lottery.index) {
            ...
            switch (lottery.place) {
				case 0:
					lottery.drawPointsYes(688);
				break;
				case 1:
					lottery.drawPointsYes(366);
				break;
				case 2:
					lottery.drawPointsYes(166);
				break;
				case 3:
					lottery.drawPointsYes(66);
				break;
				case 4:
					lottery.drawPointsYes(36);
				break;
				case 5:
					lottery.drawPointsYes(16);
				break;
				case 6:
					lottery.drawPointsYes(6);
				break;
				case 7:
					lottery.drawPointsYes(1);
				break;
			}
			lottery.prize = -1;
            lottery.times = 0;
        ...
        if (lottery.times > lottery.cycle+10 && lottery.prize == lottery.index) {
            ...
            switch(lottery.place) {
				case 0:
					...
					$('.present_img span').html('iPhone8');
					lottery.layerShow('draw_presents');
				break;
				case 1:
				...
				break;
				case 3:
					$('.hongbao_content li').eq(0).addClass('prize_bg_1');
					$('.hongbao_content .color333 span').html('&nbsp;268元&nbsp;红包');
					lottery.layerShow('draw_hongbao');
				break;
				case 4:
					$('.hongbao_content li').eq(0).addClass('prize_bg_1');
				...
				break;
				case 7:
					$('.hongbao_content li').eq(0).addClass('prize_bg_2');
	...
	getDrawRecord : function(){
		_apiDraw.getDrawRecord(lottery.headerData,1,function(res){
			...
			if (res.content.list.length == 0) {
				...
				lottery.changeColor('.record_table');
				_paging.paging('pageList',res.content.total,res.content.pageSize,function(e){
					_apiDraw.getDrawRecord(lottery.headerData,e.current,function(res){
						var bannerHtml = _td.renderHtml(record,{
							list:res.content.list
						});
						$('.record_table').html(bannerHtml);
			...
			$('.record_top .draw_num').html('---');
			$('.record_table').html('<img src="" alt="" />');
			console.log(res.msg);
		})
	},
    // 可用积分不够抽奖是的样式
